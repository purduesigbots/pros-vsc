    ...
    switch (action) {
      ...
      case "Don't Show Again":
        vscode.workspace
          ...
          .update(PROMPT_CONFIG_NAME, false, vscode.ConfigurationTarget.Global);
        break;
      case "More...":
...
export const build = async () => {
  ...
  const unsavedUris = findUnsavedUris(C_CPP_LANG_IDS);

  if (promptSetting && unsavedUris.length > 0) {
    await handleUnsavedFiles(unsavedUris);
  }
