...
export class UploadCommand extends BaseCommand {
    ...
    parseOutput = async(liveOutput: string[], process: ChildProcess): Promise<boolean> => {
        ...
        var hasError = liveOutput.some(line => {
            ...
            if (line.startsWith(PREFIX)) {
                ...
                if (jdata.type.startsWith("log") && jdata.level !== "DEBUG") {
                    ...
                    window.showWarningMessage(jdata.description, "Yes", "No").then(response => {
                        if (response === "Yes") {
                            process.stdin?.write(`{"uuid": "${jdata.uuid}", "event": "confirm"}\n`);
                        } else {
                            process.kill();
                        }
                    });
